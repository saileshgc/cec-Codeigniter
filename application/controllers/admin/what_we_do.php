...
function add_story($id=NULL)
		...
		function slide_delete($id)
	{
		...
		$this->session->set_flashdata('success_messsage', 'image deleted.');
		...
		function sliding_list()
	{
			...
			if($this->input->post('submit_slide'))
		{
			...
			{
				$prev_image = $this->input->post('prev_image');
				$image = upload_image('home_image', $this->config->item('home_images_root'), array('dest' => $this->config->item('home_images_root'), 'size' => array('w' => 400, 'h' => 375), 'ratio' => false), $prev_image);
			...
			function edit_slide($id)
	{
		...
		if($this->input->post('submit_slide'))
		{
			...
			redirect(filter_url('edit_slide'));
		}
		
		$this->load->model('sliding_model');
		

		$data['slide'] = $this->sliding_model->get_slide_by_id($id);
		//debug_array($data);die();
		$data['page_title'] = 'Edit Picture';
		$data['page'] = 'slide_add_v';
		$this->load->view('admin/main_v', $data);
	}
	...
	function add_edit_slide($id=NULL)
	{
		...
		if($id)
		{
			...
			$this->session->set_flashdata('success_message', 'Image successfully updated.');
			}
			 else
			 {
		$this->general_db_model->insert('ng_home_images', $post);
...
/*---------------------------Right Image Section---------------------------*/
	function image_right($task='list', $id=NULL)
	{
		switch($task)
		{
			case 'list':
	...
	function rimage_list()
	{
		$where ='';
		$data['total_rimages'] = $this->sliding_model->total_rimages($where);
		$data['rimages'] = $this->sliding_model->get_rimages($this->settings->item('rows_per_page'), $this->uri->segment(5), $where);
		
		create_pagination($data['total_rimages'], 5, $this->settings->item('rows_per_page'));

		$data['page_title'] = 'Home Right Image Info';
		$data['page'] = 'rimage_listing_v';
		$this->load->view('admin/main_v', $data);
		}
/*------------------Member Section--------------------------------*/	
...
/*---------------Donation Section-----------------------------*/	
	...
	function add_edit_donation($id=NULL)
	{
		if($this->input->post('submit_donar'))
		{
			$not = array('prev_image','submit_donar');
			$post = $this->general_model->get_post_array($not);
			$upload_image = $this->input->post('prev_image');
			if($_FILES['cause_image']['name'])
			{
				...
				$prev_image = $this->input->post('prev_image');
				$image = upload_image('cause_image', $this->config->item('cause_images_root'), array('dest' => $this->config->item('cause_images_root'), 'size' => array('w' => 247, 'h' => 137), 'ratio' => false), $prev_image);
				
				if($image)
					$upload_image = $post['cause_image'] = $image;
				else
					$post['cause_image'] = $prev_image;
			}
...
/*-----------------------------------Banner Section---------------------------------------*/
function banner($task='list', $id=NULL)
	{
		switch($task)
		{
			case 'list':
	...
	{
		$where ='';
		$data['total_banners'] = $this->donar_model->total_banners($where);
		$data['banners'] = $this->donar_model->get_banners($this->settings->item('rows_per_page'), $this->uri->segment(4), $where);
		
		create_pagination($data['total_banners'], 4, $this->settings->item('rows_per_page'));

		$data['page_title'] = 'Banner Info';
		$data['page'] = 'banner_list_v';
		$this->load->view('admin/main_v', $data);
		}	
	...
	function edit_banner($id)
	{
		...
		if($this->input->post('submit_banner'))
		{
			
			$this->add_edit_banner($id);
			//redirect(filter_url('edit_banner'));
			redirect(current_url());
		}
		
	
		$data['banner'] = $this->donar_model->get_banner_by_id($id);
		//debug_array($data);die();
		$data['page_title'] = 'Edit Banner Information';
		$data['page'] = 'banner_add_v';
		$this->load->view('admin/main_v', $data);
	}
	

	function add_edit_banner($id=NULL)
	{
		if($this->input->post('submit_banner'))
		{
			
			
			$not = array('prev_image','submit_banner');
			$post = $this->general_model->get_post_array($not);
			$upload_image = $this->input->post('prev_image');
			if($_FILES['banner_image']['name'])
			{
				...
				$prev_image = $this->input->post('prev_image');
				$image = upload_image('banner_image', $this->config->item('banner_images_root'), array('dest' => $this->config->item('banner_images_root'), 'size' => array('w' => 247, 'h' => 137), 'ratio' => false), $prev_image);
				
				if($image)
					$upload_image = $post['banner_image'] = $image;
				else
					$post['banner_image'] = $prev_image;
			}
		...
		if($id)
		{
			$this->general_db_model->update('ng_banner', $post,'banner_id = ' .$id);
			$this->session->set_flashdata('success_message', 'Banner Info successfully updated.');
			}
			 else
			 {
		$this->general_db_model->insert('ng_banner', $post);
		$this->session->set_flashdata('success_message', 'Banner Info successfully added.');
			 }
